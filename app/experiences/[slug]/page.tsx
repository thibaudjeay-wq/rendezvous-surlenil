            ...
            <div className="flex flex-wrap justify-center gap-x-3 gap-y-4">
              {exp.highlights.map((h, i) => {
                const displayValue = (h.value ?? '—')
                  ...
                  .replace(/pension\s+complète/i, 'Tous les repas à bord')
                  .replace(/petit-déjeuner.+d[îi]ner/i, 'Tous les repas à bord')
                  .replace(/petit-déjeuner\s+inclus/i, 'petit-déjeuner à bord')
                ...
                return (
                  <div key={i} className="flex flex-col items-center text-center gap-1">
                    <p className="text-xs font-semibold tracking-[0.06em] uppercase" style={{ color: '#C4902A' }}>
                      {displayValue}
                    </p>
